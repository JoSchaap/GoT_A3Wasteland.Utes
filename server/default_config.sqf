...
A3W_spawnBeaconCooldown = 2*60;    // Number of seconds to wait between each use of an individual spawn beacon (0 = disabled)
...
// iniDB settings
PDB_PlayerFileID = "GoT_A3Wu_";         // Player savefile prefix (if you run multiple servers, keep it the same for all of them)
PDB_ObjectFileID = "GoT_A3Wu_";         // Object savefile prefix (if you run multiple servers, change it to a unique value for each server)
...
A3W_vehicleQuantity = 60;         // Approximate number of land vehicles to be spawned in towns
...
A3W_moneyMissionTimeout = 20*60;   // Time in seconds that a Money Mission will run for, unless completed
